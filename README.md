# Iuliia Sivokho — QA Engineer

Manual QA · Web & Mobile · API Testing · HealthTech

## Обо мне

QA-инженер с коммерческим опытом в HealthTech.  
Работала единственным QA в команде: выстроила 
контроль качества с нуля, 70+ релизов без критических 
дефектов на проде.

**Ищу:** удалённую позицию QA Engineer

## Репозитории

| Проект | Описание |
|--------|---------|
| [KITACTIVE](https://github.com/lun1qa/KITACTIVE) | API-тестирование, тест-кейсы, HealthTech продукт |
| [rocket-chat-qa-portfolio](https://github.com/lun1qa/rocket-chat-qa-portfolio) | Функциональное и API-тестирование Rocket.Chat |

## Контакты

📧 y.sivokho@inbox.ru  
💼 [LinkedIn] (https://linkedin.com/in/lun1qa)  
✈️ Telegram: [@lun1qa] (https://t.me/lun1qa)
